...
def registrar_m(request):
    ...
    try:
        ...
        tipo_usuario = TipoUsuario.objects.get(pk=2)  # asumes que 2 es 'usuario'
    except (Comuna.DoesNotExist, Region.DoesNotExist, TipoUsuario.DoesNotExist):
        messages.error(request, "Región, comuna o tipo de usuario inválido.")
        return redirect('registrarse')

    # 3) Verificar si el username ya existe
    try:
        Usuario.objects.get(username=username)
        messages.error(request, "Ese usuario ya está tomado.")
        return redirect('registrarse')
    except Usuario.DoesNotExist:
        # 4) Crear el usuario porque no existía
    ...
    # 5) Intentar crear la dirección
    try:
        # Opción A: create + manejo explícito de errores
        ...
        # Opción B alternativa: get_or_create
        # direccion_obj, created = Direccion.objects.get_or_create(
        #     usuario=usuario,
        #     region=region_obj,
        #     defaults={'descripcionDir': direccion}
        # )
        # if not created:
        #     messages.warning(request, "Esa dirección ya existe para este usuario.")

    except IntegrityError:
        messages.warning(request, "No se pudo registrar la dirección: ya existe.")
        return redirect('registrarse')
...
def newProd(request):
    ...
    try:
        ...
        Producto.objects.create(nombreProducto=nombre, precioProducto=precio, especificacionProd=desc, stockProd=stock,
                                imagenProd=imagen, tipoprod=idProd2, marca=marca2)
        return redirect('menu_admin')
